...
In our research, we are utilizing both quantitative and categorical variables. Quantitative variables include the number of flights delayed and cancelled at different airports across the country. Categorical variables include month data and airport codes.
...
Our research question first requires us to create a new variable determining to help us determine when observations were recorded in term of season. The winter months include December, January, and February; the spring months include March, April, and May; the summer months include June, July, and August; and the fall months include September, October, and November. To create this new variable, we utilized the mutate() function and the if_else() function Swithin mutate().
...
Since we are focusing on the top five airports in the United States, we also wanted to filter out the data to only include observations for those five airports. These airports include Hartsfield-Jackson Atlanta International Airport (ATL), Dallas/Fort Worth International Airport (DFW), Denver International Airport (DEN), O'Hare International Airport (ORD), and Los Angeles International Airport (LAX).
...
Here, we observed that highest number of delays across the five airports occur either during the summer and winter months. ATL (Atlanta) had the highest number of delays of the five airports during the summer months while ORD (O'Hare) had the hightese number of delays during the winter months.

While this above graphs do provide us with useful insight into what time of year has the highest number of delays and cancellations, we also wanted to utilize the summarize() function to calculate means.
